...
Keep `SANDBOX_API_VERSION` at `2` while the downstream contract below remains source compatible. Change it only for an intentional downstream API break.
...
The following signature inventory records the version 2 contract; it is not a standalone Rust module. Its constants, trait implementations, types, variants, fields, constructors, methods, and `#[non_exhaustive]` markers must remain source compatible.
...
impl SandboxPolicy {
    ...
    pub fn terminal_inherited_or_created(self) -> Self;
}
...
impl SandboxRequest {
    ...
    pub fn stdin(self, mode: SandboxStdioMode) -> Self;
    pub fn stdout(self, mode: SandboxStdioMode) -> Self;
    pub fn stderr(self, mode: SandboxStdioMode) -> Self;
    pub fn lifetime(self, lifetime: SandboxLifetime) -> Self;
}

pub struct SandboxedChild { /* opaque, Send */ }
#[derive(Clone)]
pub struct SandboxedProcess { /* opaque, Send + Sync */ }
...
The behavioral contract is also part of version 2:
...
- Each standard stream independently supports native inheritance, a raw pipe, or the native null device. Only piped streams expose facade handles. Inheritance does not use forwarding tasks.
...
- `BackendDefault` does not request durable tree supervision. On Unix it controls the direct root and preserves its inherited process group and session; Windows retains its existing job-object ownership.
- `SupervisedProcessTree` is fail-closed. macOS starts an isolated session and process group, observes the root without reaping it, pins its PID through exact-group cleanup, and reaps once during retirement. Windows retains the existing job-object authority. Linux rejects this lifetime.
- Root observation is repeatable and does not retire a supervised tree. Retirement is serialized, repeatable, and preserves the root's original status after descendant cleanup.
- macOS exact-group cleanup excludes zombies, rechecks membership before every per-process signal, and retries until quiescent or a bounded deadline. A cleanup failure leaves the direct root waitable so the caller can retry without releasing the process-group identity.
- `terminate_current_process_group_members()` succeeds on macOS only when the caller is both its own process-group leader and session leader. It preserves the caller and quiesces every other live exact-group member.
- `InheritedAndCreatedOnly` is fail-closed and currently macOS-only. It denies reopening pre-existing terminal paths without closing inherited streams or blocking sandbox-created PTYs.
- The selected helper or native session and application state remain alive while any child, process controller, or stream handle needs them. Dropping the last owner performs bounded supervised cleanup.
- `SandboxRuntime` and `SandboxedProcess` are `Send + Sync`; `SandboxedChild`, `SandboxedStdin`, and `SandboxedOutput` are `Send`.

`tests/public_contract.rs` imports only `codex_sandbox_api` and is the source compatibility test. `tests/runtime_contract.rs`, `tests/stdio_contract_v2.rs`, and `tests/macos_supervision_contract.rs` cover the public process, policy, stream, and supervised-lifetime behavior against real backends where the host permits it.

## Internal Codex APIs adapted

The facade currently adapts these private dependencies:

- `codex_sandboxing::seatbelt::{create_seatbelt_command_args, create_seatbelt_command_args_with_terminal_policy}`, `CreateSeatbeltCommandArgsParams`, `MacosTerminalPolicy`, and `MACOS_PATH_TO_SEATBELT_EXECUTABLE` generate and launch the macOS Seatbelt policy. `restricted_platform_defaults_overlap_writes` exposes the smallest policy-compatibility query needed for fail-closed translation.
...
- `codex_utils_pty::process_group` supplies Unix process-group setup and signalling plus macOS non-reaping exit observation, exact-member enumeration, and bounded quiescence. The facade owns `tokio::process::Child` directly on Unix so native signal status is retained.
...
- `codex_windows_sandbox::spawn_windows_sandbox_session_for_embedding`, `WindowsSandboxEmbeddingRequest`, the typed embedding stdio configuration, and the opaque embedding process handle form the native Windows seam. That seam reuses Codex capability SID, ACL, restricted-token, private-desktop, process-launch, and job-object primitives while preserving native inherited handles, optional exact pipes, null handles, and an application-owned state directory.
...
Version 2 uses an embedding-only unelevated restricted-token session with a private desktop and non-breakaway job object. The application-owned `state_dir` replaces Codex home at the embedding seam and holds per-spawn capability SID and runtime state. The facade does not call `find_codex_home()`, write sandbox logs, or initialize normal user Codex state.
...
The current backend supports host reads, write restrictions, unrestricted network, native inherited standard handles, optional raw pipes, null handles, and job-object process-tree termination. It does not support minimal reads, read deny rules, direct network denial, interrupt delivery, terminal-path isolation, or reopening a writable child below a read-only carveout of a broader writable root.
...
- Windows minimal-read policy, read deny rules, direct network denial, interrupts, and terminal-path isolation; Windows also rejects write-read-write nested carveouts, including alias-based carveouts.
- Linux supervised process-tree ownership and terminal-path isolation.
- macOS supervision of descendants that deliberately leave the isolated process group.
...
- A target `argv[0]` distinct from the required absolute program path.
- Allocating a PTY as a public launch mode, approvals, escalation, sessions, output aggregation, and higher-level restart, timeout, or service-management policy. Native inherited PTYs and PTYs created by a sandboxed process remain supported streams.
...
- `codex-rs/sandboxing/src/seatbelt.rs`: exports a query for Seatbelt's mandatory writable runtime roots and a typed terminal-policy renderer. Existing callers remain source-compatible and do not opt into terminal isolation.
- `codex-rs/sandboxing/src/seatbelt_base_policy.sbpl`: adds `kern.boottime`, `sysctl.name.*`, and `sysctl.oidfmt.*` queries needed by current process and CPU-count tooling.
- `codex-rs/sandboxing/src/seatbelt_terminal_policy.sbpl`: contains the opt-in final terminal-path deny and sandbox-created-PTY allowance.
...
- `codex-rs/utils/pty/src/process_group.rs`: exports the macOS process-group lifecycle seam while preserving the existing one-shot signalling behavior for existing callers.
- `codex-rs/utils/pty/src/process_group/macos.rs`: implements macOS non-reaping status observation, exact-group quiescence, and exact-member cleanup with one excluded member.
...
- `codex-rs/windows-sandbox-rs/src/lib.rs`: exports the opaque embedding request, typed standard-stream configuration, process, handle, and launcher.
...
They implement strict temporary ACL leases, an embedding-only capability restricted token, native inherited or null handles, optional exact pipes, acknowledged piped-stdin writes, private desktop launch, and job-backed lifecycle. The public facade and most new code remain in `codex-rs/sandbox-api/`.
...
bazel test \
    ...
    //codex-rs/sandbox-api:sandbox-api-stdio_contract_v2-test \
    //codex-rs/sandbox-api:sandbox-api-macos_supervision_contract-test
```
...
For the downstream compatibility check, create a standalone crate whose only
Codex dependency is `codex-sandbox-api` at the immutable patch commit, then run
`cargo +1.94.0 check` in that crate. A workspace path dependency is not an
equivalent check.
