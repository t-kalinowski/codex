...
impl Supervisor {
    pub fn start(
        ...
        tokio::spawn(async move {
                ...
                && let Err(error) = handle.shutdown().await
            {
                ...
                outcome.infrastructure.cleanup_error =
                    Some(outcome.infrastructure.cleanup_error.take().map_or(
                        proxy_error.clone(),
                        |helper_error| {
                            format!("{helper_error}; managed proxy cleanup failed: {proxy_error}")
                        },
                    ));
            }
    ...
    pub async fn wait(&self, timeout: Duration) -> Result<FinalOutcome> {
        ...
        match observed {
            Ok(outcome) => outcome,
            Err(_) => {
                let root_detail = if self.process.try_root_outcome().is_some() {
                    "target root exit was observed"
                } else {
                    "target root exit was not observed"
                };
                anyhow::bail!("target retirement observation timed out; {root_detail}")
            }
        }
    }
...
fn final_outcome_from_native(native: WindowsSandboxStandaloneOutcome) -> FinalOutcome {
    FinalOutcome {
        ...
        infrastructure: InfrastructureOutcome {
            ...
            cleanup_error: native.cleanup_error,
        },
    }
}
