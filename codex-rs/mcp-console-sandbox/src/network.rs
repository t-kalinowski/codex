...
use crate::environment::apply_managed_proxy_environment;
...
pub fn validate_network_policy(policy: &NetworkPolicy) -> Result<()> {
    ...
    validate_domain_patterns(allowed_domains)?;
    validate_domain_patterns(denied_domains)?;
    if *socks_udp {
...
pub async fn prepare_network(
    ...
    match policy {
        ...
        NetworkPolicy::ManagedProxy {
            ...
            #[cfg(target_os = "macos")]
            let (proxy_environment, preserve_native_git_ssh_command) = {
                let mut proxy_environment = HashMap::new();
                let preserve_native_git_ssh_command =
                    match environment.get(OsStr::new(PROXY_GIT_SSH_COMMAND_ENV_KEY)) {
                        Some(value) => match value.to_str() {
                            Some(value) => {
                                proxy_environment.insert(
                ...
                (proxy_environment, preserve_native_git_ssh_command)
            };
            #[cfg(not(target_os = "macos"))]
            let proxy_environment = HashMap::new();
            let prepared = match proxy.prepare_for_optional_environment(proxy_environment, None) {
            ...
            let updates = prepared.env;
            #[cfg(target_os = "macos")]
            let updates = {
                let mut updates = updates;
                if preserve_native_git_ssh_command {
                    updates.remove(PROXY_GIT_SSH_COMMAND_ENV_KEY);
                }
                updates
            };
            if let Err(error) = apply_managed_proxy_environment(environment, updates) {
                let _ = handle.shutdown().await;
                return Err(error).context("managed network target environment is invalid");
            }
            Ok(PreparedNetwork {
                sandbox_policy: NetworkSandboxPolicy::Restricted,
...
fn validate_domain_patterns(patterns: &[String]) -> Result<()> {
    for pattern in patterns {
        let host = pattern
            .strip_prefix("**.")
            .or_else(|| pattern.strip_prefix("*."))
            .unwrap_or(pattern);
        let has_wildcard_prefix = host.len() != pattern.len();
        let is_ipv6_literal = host.parse::<std::net::Ipv6Addr>().is_ok();
        let contains_glob_syntax = host
            .chars()
        ...
        if host.is_empty()
            ...
            || host.contains(['/', '@', '#'])
            || host.contains(':') && !is_ipv6_literal
            || has_wildcard_prefix && is_ipv6_literal
        {
            bail!("managed domain patterns must be exact hosts, *.example.com, or **.example.com")
        }
    }
    Ok(())
}
