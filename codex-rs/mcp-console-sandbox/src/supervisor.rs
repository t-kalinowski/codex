...
mod unix {
            ...
            #[cfg(not(target_os = "linux"))]
            {
                ...
                if deadlines.graceful_ms == 0 {
                    if process_tree_exists(process_group_id) {
                        ...
                        {
                            return Err(error).context("target forced termination failed");
                        }
                    }
                    drop(process_group);
                    return Ok(());
    ...
    async fn retire_after_root(
        ...
        if let Some(deadline) = explicit_deadline {
            ...
            return force_at_expired_deadline(process_group_id, forced);
        }
        ...
        if let Err(error) = signal_terminate(process_group_id) {
            return failed_retirement(/*forced*/ false, error);
        }
        ...
        if let Err(error) = signal_kill(process_group_id) {
            return failed_retirement(/*forced*/ true, error);
        }
        forced.store(true, Ordering::Release);
        let deadline =
    ...
    #[cfg(not(target_os = "linux"))]
    fn force_at_expired_deadline(process_group_id: u32, forced: &AtomicBool) -> RetirementOutcome {
        if !process_tree_exists(process_group_id) {
            return complete_retirement(forced.load(Ordering::Acquire));
        }
        forced.store(true, Ordering::Release);
        ...
        RetirementOutcome {
            ...
            forced: true,
            error: (!complete).then(|| {
                "target process group remained alive after the requested force deadline".to_string()
            }),
        }
    }
