...
fn setup_status_from_backend_availability(
    backend_availability: &Result<(), BackendAvailabilityError>,
) -> SetupStatus {
    ...
    if let Err(error) = backend_availability {
        return SetupStatus {
...
pub fn backend_availability(state_dir: &Path) -> Result<(), BackendAvailabilityError> {
    ...
    {
        let _ = state_dir;
        return Err(BackendAvailabilityError::Backend(
            "no native Codex sandbox backend exists for this platform".to_string(),
        ));
    }
    #[cfg(target_os = "macos")]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        crate::linux_process::LinuxProcess::open(std::process::id()).map_err(|error| {
            BackendAvailabilityError::Backend(format!(
                "Linux pidfd supervision is unavailable: {error}"
            ))
        })?;
        let runner = std::env::current_exe()
            .map_err(|error| BackendAvailabilityError::Backend(error.to_string()))?;
        codex_linux_sandbox::verify_packaged_bwrap_runtime(&runner, state_dir).map_err(
            |error| match error {
                codex_linux_sandbox::PackagedBwrapRuntimeError::Companion(message) => {
                    BackendAvailabilityError::Companion(message)
                }
                codex_linux_sandbox::PackagedBwrapRuntimeError::Backend(message) => {
                    BackendAvailabilityError::Backend(message)
                }
            },
        )?;
    }
    #[cfg(target_os = "windows")]
    {
        ...
        let resource_dir = runner
            .parent()
            .ok_or_else(|| {
                BackendAvailabilityError::Backend(
                    "runner executable has no parent directory".to_string(),
                )
            })?
        ...
        .map_err(|error| BackendAvailabilityError::Companion(format!("{error:#}")))?;
    }
    let _ = state_dir;
    Ok(())
}
