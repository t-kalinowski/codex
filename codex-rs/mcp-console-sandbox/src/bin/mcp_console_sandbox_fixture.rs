...
fn main() {
    #[cfg(target_os = "linux")]
    if dispatch_linux_bwrap_fixture_if_requested() {
        return;
    }
    #[cfg(unix)]
...
#[cfg(target_os = "linux")]
fn dispatch_linux_bwrap_fixture_if_requested() -> bool {
    ...
    if arguments.as_slice() == [OsStr::new(COMPATIBILITY_QUERY)] {
        print!("{COMPATIBILITY_RESPONSE}");
        return true;
    }
    if !arguments
        .iter()
        .any(|argument| argument == OsStr::new(RUNTIME_PREFLIGHT_ARG))
    {
        return true;
    }
    ...
    match behavior.as_str() {
        "runtime_unavailable" => {
            eprintln!(
                "fixture reports native namespace setup unavailable: {}",
                "x".repeat(8192)
            );
            std::process::exit(73);
        }
        "runtime_descendant" => {
            eprintln!("fixture runtime preflight stalled");
            spawn_linux_bwrap_fixture_descendant(&executable);
            std::thread::sleep(Duration::from_secs(30));
        }
        "runtime_exit_with_descendant" => {
            eprintln!("fixture runtime preflight failed");
            spawn_linux_bwrap_fixture_descendant(&executable);
            std::process::exit(73);
        }
        behavior => panic!("unknown Linux bubblewrap test behavior: {behavior}"),
    }
    true
}

#[cfg(target_os = "linux")]
fn spawn_linux_bwrap_fixture_descendant(executable: &std::path::Path) {
    let descendant = Command::new("/bin/sleep")
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn bubblewrap fixture descendant");
    let process_id = descendant.id();
    std::fs::write(executable.with_extension("pid"), process_id.to_string())
        .expect("write bubblewrap fixture descendant process ID");
    // The enclosing runner process group owns and reaps this deliberate orphan.
...
fn dispatch_ready_loss_helper_if_requested() -> bool {
    ...
    let result = match behavior.as_deref() {
        Some("final_clean") => send_final(&arguments[1], &arguments[2], FinalBehavior::Exit),
        Some("final_holding") => send_final(&arguments[1], &arguments[2], FinalBehavior::Hold),
        _ => consume_spawn_without_ready(&arguments[1], &arguments[2]),
    };
    if let Err(error) = result {
        eprintln!("{error}");
        std::process::exit(73);
...
fn dispatch_test_compatibility_behavior(response: &str) {
    ...
    match behavior.as_str() {
        ...
        "final_clean" | "final_holding" => print!("{response}"),
...
#[cfg(windows)]
fn send_final(pipe_in: &OsStr, pipe_out: &OsStr, behavior: FinalBehavior) -> Result<(), String> {
    ...
    if matches!(behavior, FinalBehavior::Hold) {
        std::thread::sleep(Duration::from_secs(30));
    }
    Ok(())
}
