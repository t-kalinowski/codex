...
fn dispatch_ready_loss_helper_if_requested() -> bool {
    ...
    if let Err(error) = consume_spawn_without_ready(&arguments[1], &arguments[2]) {
        eprintln!("{error}");
        std::process::exit(73);
    }
    true
...
fn http_follow(host: Option<OsString>, port: Option<OsString>) -> Result<(), String> {
    ...
    let port = unicode(port, "port")?;
    let response = proxy_http_request("GET", &host, &port, "/")?;
    require_http_status(&response, 302)?;
...
fn host_process_id() -> Result<u32, String> {
    ...
    status
        ...
        .and_then(|process_ids| process_ids.split_whitespace().next())
        .ok_or_else(|| "process status is missing NSpid".to_string())?
        .parse()
        .map_err(|error| format!("invalid host process ID: {error}"))
}
