...
pub async fn start(
    ...
    #[cfg(target_os = "linux")]
    drop(prepared.bwrap_info_writer);
    ...
    drop(prepared.launch_commit_status_writer);
    #[cfg(target_os = "linux")]
    let namespace_process_id = match prepared.bwrap_info.child_process_id().await {
        Ok(process_id) => process_id,
        Err(source) => {
            drop(prepared.launch_gate);
            let _ = codex_utils_pty::process_group::kill_process_group(process_id);
            let _ = watchdog.retire().await;
            let _ = child.wait().await;
            return Err(StartError::Launch {
                source,
                supervisor: None,
    ...
    #[cfg(target_os = "linux")]
    let namespace_process = {
        let namespace_process = match crate::linux_process::LinuxProcess::open(namespace_process_id)
        {
            Ok(process) => process,
            Err(source) => {
                drop(prepared.launch_gate);
                ...
                return Err(StartError::Launch {
                    source: anyhow::Error::new(source)
                        .context("open sandbox namespace process for supervision"),
                    supervisor: None,
                });
            }
        };
        if let Err(source) = watchdog.register_namespace_process(namespace_process_id) {
            drop(prepared.launch_gate);
            let _ = namespace_process.kill();
            let _ = codex_utils_pty::process_group::kill_process_group(process_id);
            let _ = watchdog.retire().await;
            let _ = child.wait().await;
            return Err(StartError::Launch {
