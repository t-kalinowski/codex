...
outbound, loopback, and stale-rule state, refreshes ACLs, then holds the lease
   through tree retirement, confirmed command-helper exit, and proxy cleanup. A concurrent standalone runner
...
identifiers and behavior. The lease is released before the runner publishes
   the final outcome. After a native final event, the runner gives the command
   helper five seconds to exit and close its kill-on-close Job ownership. A
   helper that remains alive has its control channel closed and is then
   force-terminated at a bounded deadline; the target and Job results remain
   intact while that helper retirement is reported as a cleanup error. No final
   outcome is published, and the policy lease remains held, if helper exit still
   cannot be confirmed. Verification failures include a bounded, redacted
