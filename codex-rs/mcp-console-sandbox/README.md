...
This branch is an implementation checkpoint. Native macOS and Linux contract suites pass, but Windows is not ready to bundle: the restricted target currently stalls in Windows process initialization before target application code runs. The runner keeps the fail-closed restricting SID set while that launch boundary remains unresolved. See [REBASE.md](REBASE.md#capability-and-validation-gaps) for the current validation record.
...
| Windows  | Codex standalone restricted identity, ACL/WFP setup, elevated command helper, and non-breakaway Job Object; implementation checkpoint only | Job-owned target tree with kill-on-close retirement                                                                       |
...
The Windows process/thread DACL transition relies on one closed identity boundary: the fixed MCP Console sandbox accounts, their credentials, and their state are private infrastructure and must not be reused by external software. Protocol version 1 supports only runner-created processes under those identities; independently launched same-identity processes are outside the contract. The supported invariant combines the machine-global one-generation lease with confirmed prior-helper exit before lease release. The new helper and target stay suspended through their respective sealing gates. An administrator or a process with `SeDebugPrivilege` is outside this sandbox boundary.
...
MCP Console stages the runner and same-revision companions in one immutable private libexec directory. Unix uses:

```text
libexec/
|-- mcp-console-sandbox
`-- codex-resources/
    `-- bwrap                              # Linux only; absent on macOS
```
...
Linux constructs only `<runner-dir>/codex-resources/bwrap`, canonicalizes and validates that executable, and never consults `PATH` in embedding mode. The focused runner self-reexecutes with the reserved `codex-linux-sandbox` invocation name and pins the canonical bubblewrap and state paths in private arguments. Before reporting availability or launching a target, it runs a bounded private compatibility query and requires the exact private companion protocol-2 and Codex-release token with no extra output. It then runs a bounded no-op through that exact self-reexecution path with `platform_minimal`, explicit read access to the runner, and denied networking. This concrete-host preflight exercises the required user, PID, IPC, and network namespaces, `/proc` mount selection, seccomp, and inner target dispatch. Failure makes the backend unavailable before launch. The runner captures at most 4,096 bytes of preflight stderr for structured diagnostics and marks truncation. The preflight has an empty environment and no target application input, so those diagnostics cannot contain target credentials. The private preflight process group receives a parent-death signal and is killed and reaped after every result. This companion ABI is independent of the runner's public protocol version 1. A Bazel build additionally embeds and verifies the companion digest. Cargo packaging relies on the exact layout, closed compatibility token, and immutable private-directory trust boundary.
...
non-elevating. Policy-specific status verifies both the last successfully
applied filesystem policy and the installed fixed firewall rules through the
exact setup companion while holding a short machine-global lease. Filesystem
policy drift reports `refresh_required`; `prepare` handles that state through
the non-elevating refresh path and reports `refreshed`. A stale per-state marker
alone never reports ready. A managed-network
...
through tree retirement, confirmed command-helper exit, and proxy cleanup. A
concurrent standalone runner fails before mutation or target start. Ordinary
Codex retains its distinct identifiers and behavior. The lease is released
before the runner publishes the final outcome. A native final event ends the
command channel, so the runner closes its control writer immediately and gives
the command helper five seconds to exit and close its kill-on-close Job
ownership. A helper that remains alive is force-terminated at a bounded
deadline; the target and Job results remain intact while that helper retirement
is reported as a cleanup error. No final outcome is published, and the policy
lease remains held, if helper exit still cannot be confirmed. Verification
...
- Windows supports the `host_read_only` filesystem base but not
  `platform_minimal`; discovery reports that gap and a request for the
  unsupported base fails before setup or target creation.
