...
Companions are resolved only relative to the canonical runner executable. The Unix layout is:
...
```text
<runner-dir>\mcp-console-sandbox.exe
...
`private_desktop` boolean, defaulting to false. `false` selects the existing
`Winsta0\Default` launch path. `true` selects the existing randomly named
private desktop under `Winsta0`; that desktop lives only for the target
generation. In both modes, before the target starts, the runner temporarily
grants the suspended helper generation's unique logon SID the required
`Winsta0` access. The default mode grants the same SID to `Winsta0\Default`;
the existing private-desktop implementation grants its new desktop to that
SID. The target's restricting set does not gain `Everyone` or the persistent
sandbox account.

The station/desktop lease remains active through confirmed helper and target
Job retirement, then revokes the generation-specific ACEs and closes its
handles. Grant failure rejects launch before target start. Revocation or handle
cleanup failure is reported in `infrastructure.cleanup_error` without replacing
an otherwise valid target or retirement outcome. Neither mode creates a
terminal capability; Windows continues to accept only ordinary direct handles
and rejects terminal isolation. Arbitrary SBPL, raw backend policy text, and
