...
Job handle and other helper-owned handles are closed. After receiving the
helper's native final event, the runner waits five seconds for normal helper
exit. It then closes the private helper control channel and, if necessary,
force-terminates the helper at a bounded deadline. Delayed or forced helper
exit is reported in `infrastructure.cleanup_error` without replacing the
target or Job retirement result. If helper exit cannot be confirmed, no
