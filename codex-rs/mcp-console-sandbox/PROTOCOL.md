...
| `workspace`                        | Absolute native path              | Optional; omitted or `null` uses `cwd`; requires `extends`                                                                   | Fixed workspace used to materialize the profile and explicit project-root entries. Does not change the command cwd.                                                                                                                                                                                                                                  |
| `workspace_options`                | Object below                      | Optional; omitted or `null` uses native constructor defaults; requires `extends: ":workspace"`                               | The two upstream temporary-directory switches.                                                                                                                                                                                                                                                                                                       |
...
| `cwd`                              | String                            | Required in descriptor mode; rejected in environment mode; no `null`                                                         | Upstream `AbsolutePathBuf`: an absolute native directory, not a URI or relative path. Must exist and be accessible at launch. It also supplies the policy's working-directory context and, without an explicit `workspace`, the selected profile's project root. Environment mode uses the runner's launch cwd.                                      |
...
| `workspace_options` field | Type               | Omission/default | Meaning                                                                                   |
| ------------------------- | ------------------ | ---------------- | ----------------------------------------------------------------------------------------- |
| `exclude_tmpdir_env_var`  | Boolean; no `null` | `false`          | `true` omits the native write grant for the trusted runner launch environment's `TMPDIR`. |
| `exclude_slash_tmp`       | Boolean; no `null` | `false`          | `true` omits the native write grant for shared `/tmp`.                                    |

A selected profile supplies omitted filesystem and network policy. A supplied restricted filesystem object appends its entries to the baseline and supplies `glob_scan_max_depth`; an empty entry array does not erase baseline rules. Explicit `network` replaces the baseline network value. Explicit filesystem kinds `unrestricted` and `external-sandbox` replace the baseline filesystem completely and keep their documented enforcement meaning. Entry parsing and resolution still use upstream types. This is not a general inheritance or configuration-merging interface.

All composed project-root entries are materialized against the fixed workspace before enforcement. Subsequent target directory changes cannot move that workspace. Ordinary `cwd` special entries keep their native command-cwd meaning. Callers that replace workers must retain the captured `workspace` in each launch request.

Default metadata entries are ordinary native `read` grants, not mandatory write-denial ceilings. A more specific `write` entry can permit a protected descendant; an equal-path `write` overrides `read`, while equal-path `deny` overrides both. Array order does not determine precedence. A `read` entry also grants reads beneath a broader `deny`; it is not an independent write-only restriction. More specific read denials remain effective. These rules apply to application-supplied read entries too. The current Linux mount backend has a limitation for read grants beneath broader read denials: the ancestor mask can hide the narrower grant, and an additional nested denial can fail at mount setup. macOS honors the narrower grant in these cases. Selection preserves these native outcomes, including their diagnostics; it does not add a compatibility matcher or change backends.

Private storage is independent of built-in selection: `lifecycle.private_tmp` still owns its creation, writable data grant, environment exports, and retirement. To retain only private writable temporary storage, explicitly exclude both native shared grants:
...
Without `extends`, the existing complete-policy contract remains unchanged: both `filesystem` and `network` are required, and no profile is merged. `workspace` and `workspace_options` then have no meaning and are rejected. No custom profile names, inheritance chains, file discovery, or selector CLI option are provided.
...
| `filesystem.entries`                         | Array of entry objects | Optional, default `[]`; `null` rejected                                 | Ordered input to upstream policy resolution. Without a selected baseline, an empty restricted policy supplies no caller-granted file access; native platform defaults are not a general workspace grant.                                                                                                                                                                      |
