...
| `codex-rs/windows-sandbox-rs/src/standalone/client.rs`                 | New exact-helper launch client, suspended helper sealing, direct-handle duplication, two-phase ready/commit handshake, observation, fail-safe helper retirement, and standalone policy lease ownership.                                                                                                                                                             |
...
| `codex-rs/windows-sandbox-rs/src/standalone/process_security.rs`       | Resolve the real runner and sandbox token identities, reject identity reuse, and install the runner- and `SYSTEM`-only protected helper process owner and DACL before helper execution.                                                                                                                                                                              |
