...
| `.github/workflows/mcp-console-sandbox.yml`                            | Run package-scoped argument-comment lint on Linux, strict runner Clippy on every host, the executable contract suite, focused modified-sandbox-crate tests, configured Bazel contract target, and complete release runner-plus-companion smoke tests on native Linux, macOS, and Windows; serialize native Windows tests around the standalone policy lease and skip only two confirmed unchanged base Seatbelt path-spelling failures on macOS. |
| `defs.bzl`                                                             | Allow one focused crate target to replace generated local-crate dependencies, supply private build and compiler environments, stamp its library, and keep binary-local compile data, flags, compiler environment, and tags scoped while leaving ordinary target behavior unchanged.                                                                                                                                                              |
| `scripts/list-bazel-clippy-targets.sh`                                 | Exclude only the runner's explicitly config-required Bazel tests from ordinary unstamped Clippy discovery; the focused Cargo Clippy gate and configured Bazel contract target cover them.                                                                                                                                                                                                                                                        |
| `tools/argument-comment-lint/list-bazel-targets.sh`                    | Exclude only the runner's config-required manual Rust tests from the unstamped repository-wide Bazel lint. The workflow's package-scoped Cargo lint covers the library, executable, fixture, and every test target.                                                                                                                                                                                                                              |
...
| `codex-rs/linux-sandbox/src/embedding.rs`                              | New closed embedding facade for runner-relative bubblewrap and state-directory selection, including typed companion/backend failures, bounded concrete-host diagnostics, and the native preflight's parent-death, private-process-group, and non-reaping exit observation.                                                                                                                                                                       |
...
| `codex-rs/linux-sandbox/src/proxy_lifecycle.rs`                        | Share the crate's existing parent-death setup with the embedding preflight and return a raw process-gone error when the parent has already exited.                                                                                                                                                                                                                                                                                               |
...
| `codex-rs/windows-sandbox-rs/src/acl.rs`                               | Add checked stale-ACE revocation for persistent standalone desired-state reconciliation while retaining the existing best-effort API for ordinary callers.                                                                                                                                                                                                                                                                                       |
...
| `codex-rs/windows-sandbox-rs/src/bin/setup_main/win.rs`                | Use the crate-local wire-compatible metrics shape and closed policy namespace, reconcile persistent standalone deny-write ACEs, and fail initial as well as refresh setup when required ACL mutations fail.                                                                                                                                                                                                                                      |
...
| `codex-rs/windows-sandbox-rs/src/bin/setup_main/win/sandbox_users.rs`  | Provision the exact identities and group selected by the closed policy namespace and record the last successfully applied standalone filesystem policy.                                                                                                                                                                                                                                                                                          |
| `codex-rs/windows-sandbox-rs/src/deny_read_state.rs`                   | Make persistent deny-read removal checked so setup cannot record a stale ACE as revoked after an ACL API failure.                                                                                                                                                                                                                                                                                                                                |
| `codex-rs/windows-sandbox-rs/src/deny_write_state.rs`                  | Reconcile the persistent deny-write paths owned by each standalone workspace capability SID across policy generations.                                                                                                                                                                                                                                                                                                                           |
| `codex-rs/windows-sandbox-rs/src/desktop.rs`                           | Share the existing exact private-desktop access mask with the standalone logon-session access lease; existing desktop creation and callers are unchanged.                                                                                                                                                                                                                                                                                        |
...
| `codex-rs/windows-sandbox-rs/src/setup.rs`                             | Accept exact setup helpers, explicit state, and a backward-compatible closed policy namespace; record the successful standalone filesystem generation; contain standalone setup in the named Job; and invoke the read-only firewall verifier with bounded, environment-cleared diagnostics.                                                                                                                                                      |
...
| `codex-rs/windows-sandbox-rs/src/standalone.rs`                        | New public low-level standalone types and facade exports, including a helper-cleanup result separate from target and Job retirement.                                                                                                                                                                                                                                                                                                             |
| `codex-rs/windows-sandbox-rs/src/standalone/client.rs`                 | New exact-helper launch client, suspended helper process and initial-thread sealing, trusted runner-SID handoff, direct-handle duplication, two-phase ready/commit handshake, observation, confirmed bounded helper-process exit before final publication, fail-safe helper retirement, and standalone policy lease ownership.                                                                                                                   |
...
| `codex-rs/windows-sandbox-rs/src/standalone/helper.rs`                 | New capability-restricted target helper with suspended pre-commit creation, a gated and sealed control thread, non-breakaway Job ownership, limited rights against infrastructure processes, retained-handle Job-member retirement observation, and separate root/final outcomes.                                                                                                                                                                |
...
| `codex-rs/windows-sandbox-rs/src/standalone/process_security.rs`       | Resolve and validate exact token identities, reject runner/sandbox identity reuse, and install protected runner- and `SYSTEM`-only process/thread DACLs. Helper-owned threads also deny implicit `OWNER RIGHTS` so the sandbox identity cannot reopen them for control access.                                                                                                                                                                   |
| `codex-rs/windows-sandbox-rs/src/standalone/setup.rs`                  | New read-only status, idempotent prepare/refresh, exact-resource, ACL, identity, and firewall adapter fixed to the MCP Console namespace, including truthful filesystem-generation `refresh_required` reporting and non-elevating refresh.                                                                                                                                                                                                       |
| `codex-rs/windows-sandbox-rs/src/standalone/window_access.rs`          | Grant the suspended helper generation's unique logon SID to `Winsta0` and the selected desktop, retain the non-inheritable lease through confirmed tree retirement, and revoke it with structured cleanup reporting.                                                                                                                                                                                                                             |
| `codex-rs/windows-sandbox-rs/src/standalone/wire.rs`                   | New bounded, versioned private helper framing with native UTF-16 values, the exact trusted runner SID, and closed ready/commit messages.                                                                                                                                                                                                                                                                                                         |
...
| `codex-rs/windows-sandbox-rs/src/token.rs`                             | Add the standalone restricted-token constructor whose write-restricting SIDs are the active filesystem capabilities, the session-scoped logon SID required for desktop initialization, and optional managed-network identity. The token-user and Everyone SIDs remain excluded, and existing Codex constructors are unchanged.                                                                                                                   |
| `codex-rs/windows-sandbox-rs/src/token_tests.rs`                       | Verify the exact standalone restricting SID set, including its logon-only base identity, and retain coverage of the existing token shapes.                                                                                                                                                                                                                                                                                                       |
| `codex-rs/windows-sandbox-rs/src/wfp.rs`                               | Preserve the ordinary WFP wrapper, add namespace-specific filter selection and exact persisted-policy verification for standalone setup, and build the `ALE_USER_ID` match descriptor from the resolved SID with the low-level ACL and self-relative security-descriptor APIs.                                                                                                                                                                   |
...
Bazel supplies `CODEX_BWRAP_SHA256`, and the helper reopens and executes the verified file through `/proc/self/fd`. Before discovery or launch succeeds, both build paths execute the exact companion with a two-second, 1,024-byte private query and require the exact protocol-2 and workspace-release token, zero exit, empty stderr, and no extra output. They then execute a bounded no-op through the exact self-reexecution and inner target path with `platform_minimal`, an explicit runner-read rule, and restricted networking. This concrete-host preflight checks the required user, PID, IPC, and network namespaces, `/proc` mount selection, seccomp, and target dispatch. It owns a private process group, observes completion without reaping the group leader, kills the group after every result, and uses a parent-death signal if the runner exits first. It captures and drains at most 4,096 diagnostic bytes under the same deadline, marks truncation, and runs with an empty environment and no target application input. A failed or timed-out preflight reports `backend_unavailable`; missing or incompatible companion resources report `companion_missing`. Both fail before target launch. Plain Cargo builds do not embed the additional digest; their compatibility and identity boundary is the closed token plus the immutable private layout.
...
Setup status is read-only and cannot display UAC. It compares the requested filesystem policy with the last successfully applied ACL generation and verifies the installed fixed standalone firewall rules under a short global lease. Filesystem drift reports `refresh_required`; `prepare` applies that drift through the non-elevating refresh path and reports `refreshed`. Initial identity or firewall preparation is the only UAC-capable path. Every required ACL mutation and checked stale-denial revocation must succeed before the generation marker advances. Launch verifies the firewall generation under its lifetime lease and then refreshes ACL state before target creation. The online identity implements unrestricted networking; the offline identity carries WFP denied/managed confinement, proxy ports, local-binding policy, and an optional proxy restricting SID.

The fixed MCP Console identities, group, firewall names, WFP keys, and mutexes are disjoint from ordinary Codex policy objects. They permit one active standalone policy generation machine-wide. Standalone launch acquires its own lifetime lease, verifies the exact outbound, loopback, port-complement, and stale-rule state through the setup implementation, refreshes ACLs, rechecks local setup readiness, and holds the lease through Job retirement, confirmed command-helper exit, and managed-proxy cleanup. Busy or abandoned acquisition fails before target start; abandonment requires setup `prepare`. Ordinary Codex setup retains its existing identifiers and behavior. The fixed identities, their credentials, and their state are private infrastructure. The supported protocol boundary permits only runner-created processes under those identities; independently launched same-identity processes are outside the contract. The helper's suspended post-creation DACL sealing and gated control-thread sealing rely on that boundary. Each rolling patch must preserve the machine-global one-generation lease and confirm prior-helper exit before releasing it. Administrators and `SeDebugPrivilege` are outside the sandbox authority model.
...
The runner creates each command helper suspended, immediately seals the helper process and initial-thread DACLs, and then reads that generation's unique logon SID through its already-open process handle. A non-inheritable lease temporarily grants that SID the documented access to `Winsta0` and, for the default path, `Winsta0\Default`. The existing private-desktop implementation creates and grants its random desktop to the same logon SID, so the lease needs only the station for `private_desktop`. The target's restricting SID set remains the active filesystem capabilities, optional managed-network identity, and this generation-scoped logon SID; it does not include `Everyone` or the persistent sandbox account. The lease remains live until the helper and target Job have retired. Revocation and station/desktop handle failures are reported separately as cleanup failures after a valid target outcome.

The exact command helper receives native UTF-16 target arguments and values, duplicates ordinary inherited or caller-passed handles directly, and creates the target suspended in a non-breakaway kill-on-close Job. It reports `Ready` over a separate private bounded pipe. The outer runner installs supervision and releases its stream copies before sending `CommitLaunch`; only then does the helper resume the target and return `Committed`. Any uncertainty after the `Spawn` request may have been sent consumes the generation. Root-exit and final events remain separate. The helper releases its target thread handle immediately after resume and its target process handle after recording root exit, before observing Job retirement. It queries Job member IDs, verifies live members, accepts already-signaled members from the Job snapshot, and retains every native process handle until a fixed-point scan finds every observed member exited and no new member. Retaining each handle keeps its process identifier bound to that process object throughout the scan. This observes process liveness without waiting for unrelated external references to release terminated process objects. Descendants receive a natural retirement grace period, then the helper force-terminates and observes the Job. The native final event ends the helper command channel, so the runner closes its control writer immediately and confirms command-helper process exit before releasing the policy lease or publishing protocol `final`; confirmed forced helper retirement is a cleanup result and cannot replace the valid target or Job result. An unconfirmed helper exit keeps `final` unobservable and retains the policy lease. Helper input EOF, control failure, owner drop, and runner failure retire the Job, including while the target is still suspended before commit.
...
- Windows supports `host_read_only` but not `platform_minimal`; discovery reports that base as unsupported and validation rejects it before setup or target creation.
...
- Windows is not ready to bundle from this checkpoint. The fail-closed standalone restricting set allows helper and target creation but the target stalls during Windows process initialization before application code runs. Adding `Everyone` clears the stall but also admits unrelated Everyone-writable host paths, so that broader identity is intentionally excluded.
...
- Native Linux executable and sandbox tests ran in a native Linux container. Native Windows executable tests ran through the rolling-patch workflow's Windows host and exposed the launch defect above.
...
- the native macOS package suite: 75 of 75 tests passed, with no skips or nextest leak classifications;
- the native Linux runner suite: 83 of 83 tests passed, with no skips, retries, nextest leak classifications, or remaining runner/bubblewrap processes;
- the native Linux sandbox suite: 158 of 158 tests passed, with no skips or retries;
...
- native Windows runner run 33168092418: 18 of 22 tests passed and four launch-dependent tests stalled before target application code ran;
- native Windows diagnostic run 33170211579: Windows Wait Chain Traversal identified the initial target thread waiting on the CSRSS ALPC endpoint `\\Sessions\\2\\Windows\\ApiPort`; the diagnostic instrumentation is not part of this patch;
- native Windows run 33166584103 with `Everyone` temporarily present in the restricting set: 19 of 22 tests passed, but the explicit Everyone-writable-path regression failed, confirming that this apparent launch fix widens filesystem authority and cannot ship;
...
The checked-in workflow runs the executable suite and focused tests for every modified native sandbox crate on Ubuntu 24.04, macOS 14, and Windows Server 2022 hosts. The macOS and Linux jobs pass. Windows remains intentionally recorded as incomplete until the restricted target can finish native process initialization without adding a broad restricting SID.
...
just test -p codex-windows-sandbox --no-default-features
...
cargo clippy -p codex-linux-sandbox --all-targets --all-features -- -D warnings
cargo clippy -p codex-bwrap --all-targets --all-features -- -D warnings
