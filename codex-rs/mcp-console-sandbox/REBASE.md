...
| `codex-rs/linux-sandbox/src/embedding.rs`                              | New closed embedding facade for runner-relative bubblewrap and state-directory selection, including the bounded concrete-host native preflight and its parent-death, private-process-group, and non-reaping exit observation.                                                                                                                                                                                                                    |
...
Bazel supplies `CODEX_BWRAP_SHA256`, and the helper reopens and executes the verified file through `/proc/self/fd`. Before discovery or launch succeeds, both build paths execute the exact companion with a two-second, 1,024-byte private query and require the exact protocol-2 and workspace-release token, zero exit, empty stderr, and no extra output. They then execute a bounded no-op through the exact self-reexecution and inner target path with `platform_minimal`, an explicit runner-read rule, and restricted networking. This concrete-host preflight checks the required user, PID, IPC, and network namespaces, `/proc` mount selection, seccomp, and target dispatch. It owns a private process group, observes completion without reaping the group leader, kills the group after every result, and uses a parent-death signal if the runner exits first. A failed or timed-out preflight reports the backend unavailable before target launch. Plain Cargo builds do not embed the additional digest; their compatibility and identity boundary is the closed token plus the immutable private layout.
