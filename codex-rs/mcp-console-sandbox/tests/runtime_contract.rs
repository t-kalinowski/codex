...
#[test]
fn watchdog_cleanup_failure_does_not_replace_a_valid_target_outcome() {
    let target = fixture_target(&["copy"]);
    let (mut runner, io) = Runner::spawn(&target, &[], true);
    let io = io.expect("target streams");
    assert_eq!(
    ...
    let outcome = runner.request(wait_request(104));
    assert_target_exit(&outcome, 0);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert_eq!(outcome["outcome"]["infrastructure"]["error"], Value::Null);
    assert!(
        outcome["outcome"]["infrastructure"]["cleanup_error"]
            .as_str()
            .is_some_and(|error| error.contains("watchdog")),
        "{outcome}"
    );
}
