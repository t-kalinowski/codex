...
#[cfg(target_os = "linux")]
#[test]
fn packaged_bwrap_without_runtime_namespace_support_is_unavailable() {
    let companion = std::path::PathBuf::from(fixture_target(&["exit", "0"]).remove(0));
    let target_directory = TempDir::new().expect("target directory");
    let marker = target_directory.path().join("target-started");
    let target = fixture_target(&[
        "write",
    ...
    set_linux_companion_behavior(&runner, "runtime_unavailable");
    ...
    let detail = capabilities["capabilities"]["setup"]["detail"]
        .as_str()
        .expect("unavailable backend detail");
    assert!(detail.contains("namespace preflight"), "{capabilities}");
    assert!(
        detail.contains("native namespace setup unavailable"),
        "{capabilities}"
    );
    assert!(detail.contains("diagnostic truncated"), "{capabilities}");
    assert!(detail.len() < 4300, "{capabilities}");
...
#[cfg(target_os = "linux")]
#[test]
fn packaged_bwrap_runtime_preflight_retires_a_descendant() {
    let companion = std::path::PathBuf::from(fixture_target(&["exit", "0"]).remove(0));
    let target = fixture_target(&["exit", "0"]);
    let (mut runner, _) = Runner::spawn_with_companion(&target, &companion, /*with_io*/ false);
    let packaged_companion = set_linux_companion_behavior(&runner, "runtime_descendant");
    ...
    assert!(
        capabilities["capabilities"]["setup"]["detail"]
            .as_str()
            .is_some_and(|detail| {
                detail.contains("preflight timed out")
                    && detail.contains("runtime preflight stalled")
            }),
        "{capabilities}"
    );
    let descendant_process_id = std::fs::read_to_string(&descendant_process_id_path)
        .expect("read runtime preflight descendant process ID")
        .parse::<i32>()
        .expect("runtime preflight descendant process ID");
    wait_for_linux_process_exit(descendant_process_id, std::time::Duration::from_secs(2));
}

#[cfg(target_os = "linux")]
#[test]
fn packaged_bwrap_runtime_failure_retires_a_descendant() {
    let companion = std::path::PathBuf::from(fixture_target(&["exit", "0"]).remove(0));
    let target = fixture_target(&["exit", "0"]);
    let (mut runner, _) = Runner::spawn_with_companion(&target, &companion, /*with_io*/ false);
    let packaged_companion = set_linux_companion_behavior(&runner, "runtime_exit_with_descendant");
    let descendant_process_id_path = packaged_companion.with_extension("pid");
    ...
    assert!(
        capabilities["capabilities"]["setup"]["detail"]
            .as_str()
            .is_some_and(|detail| {
                detail.contains("preflight failed") && detail.contains("runtime preflight failed")
            }),
        "{capabilities}"
    );
    let descendant_process_id = std::fs::read_to_string(&descendant_process_id_path)
        .expect("read failed preflight descendant process ID")
        .parse::<i32>()
        .expect("failed preflight descendant process ID");
    wait_for_linux_process_exit(descendant_process_id, std::time::Duration::from_secs(2));
}
