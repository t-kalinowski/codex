...
fn passed_streams_are_byte_transparent_independent_and_closed_by_the_runner() {
    ...
    let (mut runner, io) = Runner::spawn(&target, &[], true);
    ...
    let outcome = runner.request(wait_request(2));
    assert_target_exit(&outcome, 0);
...
fn large_simultaneous_stdout_and_stderr_do_not_deadlock() {
    ...
    let outcome = runner.request(wait_request(11));
    assert_target_exit(&outcome, 0);
...
fn null_streams_are_independent_from_the_control_channel() {
    let target = fixture_target(&["copy"]);
    let (mut runner, _) = Runner::spawn(&target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&runner.request(wait_request(21)), 0);
}
...
fn inherited_output_endpoints_reach_eof_while_runner_remains_resident() {
    ...
    assert_target_exit(&runner.request(wait_request(23)), 0);
...
fn inherited_stdin_is_byte_transparent_and_reaches_target_eof() {
    ...
    assert_target_exit(&runner.request(wait_request(242)), 0);
}
...
fn post_commit_exec_failure_consumes_the_target_generation() {
    ...
    let (mut runner, _) = Runner::spawn(&[target.into_os_string()], &[], false);
    let response = runner.request(default_launch(
        21,
        directory.path(),
        json!([]),
    ...
    let outcome = runner.request(wait_request(23));
...
fn linux_launch_does_not_expose_a_namespace_local_root_process_id() {
    ...
    assert_target_exit(&runner.request(wait_request(26)), 0);
}
...
fn target_exit_125_remains_a_target_outcome() {
    ...
    let outcome = runner.request(wait_request(28));
    assert_target_exit(&outcome, 125);
...
fn launch_bridge_failure_is_infrastructure_not_a_target_outcome() {
    ...
    let outcome = runner.request(wait_request(30));
    assert_eq!(outcome["type"], "final");
    assert_eq!(outcome["outcome"]["target"], Value::Null);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert!(
...
fn missing_packaged_bwrap_is_unavailable_and_fails_before_target_start() {
    ...
    let (mut runner, _) = Runner::spawn_without_companion(&target, &[], false);
...
fn incompatible_packaged_bwrap_is_unavailable_and_fails_before_target_start() {
    ...
    let (mut runner, _) = Runner::spawn_with_incompatible_companion(&target, &[], false);
...
fn packaged_bwrap_compatibility_is_bounded_and_exact() {
    ...
    for (name, body, expected_detail) in cases {
        ...
        let (mut runner, _) = Runner::spawn_with_companion(&target, &companion, false);
        let capabilities = runner.request(json!({
            "type": "discover",
            "id": 26,
            "protocol_version": support::PROTOCOL_VERSION,
...
fn filesystem_write_deny_and_nested_read_precedence_are_enforced() {
    ...
    assert_write_outcome(root.path().join("allowed"), root.path(), rules.clone(), 0);
    assert_write_outcome(denied.join("blocked"), root.path(), rules.clone(), 73);
    assert_write_outcome(read_only.join("blocked"), root.path(), rules, 73);
}
...
fn filesystem_deny_covering_target_executable_fails_before_launch() {
    ...
    let (mut runner, _) = Runner::spawn(&command, &[], false);
    let response = runner.request(default_launch(
        29,
        &workspace,
        json!([
...
fn filesystem_read_roots_and_denials_do_not_widen_platform_minimal() {
    ...
    assert_read_outcome(&target, "platform_minimal", json!([]), 73);
    assert_read_outcome(
        &target,
        "platform_minimal",
        json!([{
    ...
    assert_read_outcome(&target, "platform_minimal", denied_rules.clone(), 73);
    assert_read_outcome(&target, "host_read_only", denied_rules, 73);
}
...
fn missing_path_behavior_is_explicit_and_fail_closed() {
    ...
    let (mut error_runner, _) = Runner::spawn(&target, &[], false);
    ...
    let (mut ignore_runner, _) = Runner::spawn(&target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&ignore_runner.request(wait_request(32)), 0);
}

#[test]
fn runner_state_is_protected_from_broad_writes_and_specific_overrides() {
    let target = fixture_target(&["exit", "0"]);
    let (mut override_runner, _) = Runner::spawn(&target, &[], false);
    ...
    let (mut runner, _) = Runner::spawn_with_state(&target, &[], false, state_dir);
    ...
    assert_eq!(response["type"], "launch_accepted");
    assert_target_exit(&runner.request(wait_request(42)), 73);
...
fn working_directory_and_both_filesystem_bases_launch() {
        ...
        .enumerate()
    {
        ...
        assert_target_exit(&runner.request(wait_request(index as u64 + 60)), 0);
...
fn host_read_only_denies_host_writes_without_creating_the_file() {
    ...
    assert_target_exit(&runner.request(wait_request(691)), 73);
...
fn denied_and_unrestricted_network_modes_do_not_fallback() {
    ...
    let (mut denied_runner, _) = Runner::spawn(&target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&denied_runner.request(wait_request(71)), 73);

    let accept = std::thread::spawn(move || listener.accept().expect("accept connection"));
    let (mut unrestricted_runner, _) = Runner::spawn(&target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&unrestricted_runner.request(wait_request(73)), 0);
...
fn managed_proxy_enforces_its_reported_local_binding_behavior() {
    ...
    #[cfg(target_os = "linux")]
    let expected_exit = 0;
    assert_target_exit(&runner.request(wait_request(76)), expected_exit);
}
...
fn managed_proxy_preserves_unrelated_non_unicode_target_environment() {
    ...
    let (mut runner, io) = Runner::spawn_with_native_environment(&target, &environment, true);
    let mut io = io.expect("target streams");
    assert_eq!(
        runner.request(default_launch(
            77,
    ...
    assert_target_exit(&runner.request(wait_request(78)), 0);
    assert_eq!(
...
fn non_unicode_loader_variable_name_is_rejected_before_target_launch() {
    ...
    let (mut runner, _) = Runner::spawn_with_native_environment(&target, &environment, false);
    let response = runner.request(default_launch(
        781,
        root.path(),
...
fn managed_proxy_injects_the_documented_environment_and_removes_private_values() {
    ...
    let (mut runner, io) = Runner::spawn(&target, &environment, true);
    let mut io = io.expect("target streams");
    assert_eq!(
        runner.request(default_launch(
            79,
    ...
    drop(io.stdin);
    assert_target_exit(&runner.request(wait_request(791)), 0);
...
fn managed_socks_changes_only_the_documented_all_and_ftp_proxy_aliases() {
    ...
    drop(io.stdin);
    assert_target_exit(&runner.request(wait_request(793)), 0);
    let values = decode_optional_values(&read_all(&mut io.stdout));
    let environment = keys
        .into_iter()
        .zip(values)
...
fn managed_proxy_allows_a_domain_while_confining_direct_egress() {
    ...
    drop(io.stdin);
    assert_target_exit(&runner.request(wait_request(81)), 0);
    ...
    assert_target_exit(&runner.request(wait_request(83)), 73);
    let error = listener
        .accept()
...
fn managed_proxy_applies_domain_policy_to_redirected_requests() {
    ...
    assert_target_exit(&runner.request(wait_request(841)), 73);
...
fn managed_proxy_limited_access_allows_get_and_blocks_post_before_origin() {
    ...
    let (mut get_runner, _) = Runner::spawn(&get_target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&get_runner.request(wait_request(843)), 0);
    ...
    let (mut post_runner, _) = Runner::spawn(&post_target, &[], false);
    assert_eq!(
    ...
    assert_target_exit(&post_runner.request(wait_request(845)), 73);
...
fn managed_proxy_enforces_domain_denials_and_supports_socks() {
    ...
    let (mut denied_runner, _) = Runner::spawn(&denied_target, &[], false);
    ...
    assert_eq!(response["type"], "launch_accepted");
    assert_target_exit(&denied_runner.request(wait_request(85)), 73);
    ...
    assert_target_exit(&runner.request(wait_request(87)), 0);
...
fn managed_socks_preserves_a_non_unicode_git_ssh_command() {
    ...
    drop(io.stdin);
    assert_target_exit(&runner.request(wait_request(881)), 0);
    assert_eq!(
        decode_values(&read_all(&mut io.stdout)),
        vec![native_value.as_slice()]
    );
...
fn managed_proxy_uses_a_trusted_upstream_and_closes_its_listener() {
    ...
    let (mut runner, _) = Runner::spawn(&target, &[("HTTP_PROXY", &upstream_url)], false);
    let mut network = managed_network(json!(["example.com"]), json!([]));
    ...
    assert_target_exit(&runner.request(wait_request(89)), 0);
    ...
    drop(io.stdin);
    assert_target_exit(&runner.request(wait_request(891)), 0);
...
fn managed_proxy_applies_typed_unix_socket_policy() {
    ...
    assert_target_exit(&runner.request(wait_request(893)), 0);
...
fn terminal_isolation_allows_new_ptys_and_denies_preexisting_device_reopen() {
    ...
    assert_target_exit(&preserve_runner.request(wait_request(898)), 0);

    let (mut isolate_runner, _) = Runner::spawn(&target, &[], false);
    let mut launch = default_launch(
        899,
        std::env::current_dir()
            .expect("current directory")
    ...
    assert_target_exit(&isolate_runner.request(wait_request(900)), 73);
    drop((master, slave));
}
...
fn passed_pty_endpoints_remain_native_terminal_streams() {
    ...
    assert_target_exit(&runner.request(wait_request(902)), 0);
}
...
fn inherited_terminal_streams_remain_terminals() {
    ...
    master
        ...
        .expect("read terminal status");
    assert_eq!(tty_status, [1, 1, 1]);
    assert_target_exit(&runner.request(wait_request(904)), 0);
}
...
fn preserve_keeps_the_inherited_controlling_terminal() {
    ...
    assert_target_exit(&runner.request(wait_request(906)), 0);
}

#[test]
fn lifecycle_reports_exit_signal_interrupt_and_bounded_retirement() {
    assert_lifecycle_target(&["exit", "17"], None, 17, None);
    assert_lifecycle_target(&["signal", "15"], None, 0, Some(15));
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let outcome = runner.request(wait_request(92));
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let outcome = runner.request(wait_request(920));
        assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    ...
    drop(io.stdin);
    let outcome = runner.request(wait_request(94));
    assert_target_exit(&outcome, 0);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
...
fn target_output_reaches_eof_during_root_exit_grace() {
    ...
    let outcome = runner.request(wait_request(938));
    assert_target_exit(&outcome, 0);
}
...
fn linux_session_escaping_descendant_survives_grace_before_forced_retirement() {
    ...
    let outcome = runner.request(wait_request(942));
    assert_target_exit(&outcome, 0);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert_eq!(outcome["outcome"]["retirement"]["forced"], true);
    assert_eq!(outcome["outcome"]["infrastructure"]["error"], Value::Null);
...
fn forced_termination_without_a_target_frame_is_an_infrastructure_failure() {
    ...
    let outcome = runner.request(wait_request(97));
    assert_eq!(outcome["outcome"]["target"], Value::Null);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert_eq!(outcome["outcome"]["retirement"]["forced"], true);
    assert!(
...
fn zero_deadline_termination_does_not_leave_the_target_group_running() {
    ...
    let outcome = runner.request(wait_request(953));
...
fn retired_generation_rejects_signals_while_cleanup_is_blocked() {
    ...
    assert_target_exit(&runner.request(wait_request(102)), 0);
}
...
fn valid_target_outcome_is_preserved_when_watchdog_cleanup_fails() {
    ...
    let outcome = runner.request(wait_request(1023));
...
fn watchdog_exit_forces_retirement_without_replacing_a_valid_target_outcome() {
    ...
    let outcome = runner.request(wait_request(105));
    assert_target_exit(&outcome, 0);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert_eq!(outcome["outcome"]["retirement"]["forced"], true);
    assert!(
...
fn state_machine_rejects_invalid_and_second_generation_transitions() {
    ...
    assert_target_exit(&runner.request(wait_request(113)), 0);
...
fn explicit_termination_retires_a_root_exited_descendant() {
    ...
    let outcome = runner.request(wait_request(119));
    assert_target_exit(&outcome, 0);
    assert_eq!(outcome["outcome"]["retirement"]["complete"], true);
    assert_eq!(outcome["outcome"]["retirement"]["forced"], true);
    wait_for_linux_process_handle_exit(&descendant_process, std::time::Duration::from_secs(2));
...
fn runner_process_group_termination_still_retires_the_target_group() {
    let target = fixture_target(&["ignore-term", "10000"]);
    let (mut runner, io) = Runner::spawn_in_new_process_group(&target, &[], true);
    let response = runner.request(default_launch(
        121,
        std::env::current_dir()
            .expect("current directory")
...
fn assert_write_outcome(path: std::path::PathBuf, cwd: &Path, rules: Value, exit_code: i32) {
    ...
    assert_target_exit(&runner.request(wait_request(2)), exit_code);
}
