...
mod unix {
    ...
    impl Runner {
        fn spawn(target: &[&str]) -> Self {
            ...
            Self::spawn_native(&target, &[], /*target_io*/ None).0
        }
        ...
        fn assert_control_closed(&mut self) {
            self.child.wait().expect("wait for runner");
            let mut byte = [0_u8; 1];
            assert_eq!(self.control.read(&mut byte).expect("read control EOF"), 0);
        }
    }
    ...
    fn run_with_closed_control(
        ...
        let cleanup_directory = TempDir::new().expect("cleanup directory");
        run_with_closed_control_and_cleanup(
            runner_executable,
            state_directory,
    ...
    fn run_with_closed_control_and_cleanup(
        runner_executable: &std::path::Path,
        state_directory: &std::path::Path,
        cleanup_directory: &std::path::Path,
    ) -> Output {
        let (control, child_control) = UnixStream::pair().expect("control socket pair");
        let child_control_fd = child_control.as_raw_fd();
    ...
    #[test]
    fn cleanup_directory_must_be_absolute_existing_and_separate() {
        let runner_executable = RunnerExecutable::packaged();
        let state_directory = TempDir::new().expect("state directory");

        let relative = run_with_closed_control_and_cleanup(
            runner_executable.path(),
            state_directory.path(),
            std::path::Path::new("relative-cleanup-directory"),
        );
        assert!(!relative.status.success());

        let missing = state_directory.path().with_extension("missing-cleanup");
        let missing_output = run_with_closed_control_and_cleanup(
            runner_executable.path(),
            state_directory.path(),
            &missing,
        );
        ...
        let shared_directory = TempDir::new().expect("shared state and cleanup directory");
        let shared = run_with_closed_control_and_cleanup(
            runner_executable.path(),
    ...
    #[test]
    fn private_launch_bridge_refuses_unsandboxed_execution() {
        let runner_executable = RunnerExecutable::packaged();
        let directory = TempDir::new().expect("bridge test directory");
        let canary_path = directory.path().join("canary");
        let canary = std::fs::OpenOptions::new()
            .read(true)
            .write(true)
            .create_new(true)
            .open(&canary_path)
            .expect("create bridge canary");
        ...
        let target_arguments = {
            ...
            let mut file = tempfile::tempfile().expect("target argument file");
            let target = [
                fixture.as_os_str(),
                std::ffi::OsStr::new("write"),
                marker.as_os_str(),
                std::ffi::OsStr::new("started"),
            ];
            file.write_all(b"MCA1").expect("argument magic");
            file.write_all(&(target.len() as u32).to_be_bytes())
                .expect("argument count");
            for argument in target {
                let bytes = argument.as_bytes();
                file.write_all(&(bytes.len() as u32).to_be_bytes())
                    .expect("argument length");
                file.write_all(bytes).expect("argument bytes");
            }
            file.rewind().expect("rewind target arguments");
            file
        };
        ...
        let mut command = Command::new(runner_executable.path());
        command
            .arg("--mcp-console-private-launch-bridge")
        ...
        let status = command.status().expect("run private launch bridge");
    ...
    fn discovery_reports_version_revision_backend_and_contract_limits() {
        ...
        assert_eq!(
            response["capabilities"]["lifecycle"]["full_tree_retirement"],
            true
        );
    }
    ...
    #[test]
    fn non_unicode_target_argument_matches_the_native_backend_contract() {
        let fixture = cargo_bin("mcp-console-sandbox-fixture").expect("fixture binary");
        let target = vec![
            fixture.into_os_string(),
            OsString::from("argv"),
            OsString::from_vec(vec![b'n', b'o', b'n', 0xff, b'u', b't', b'f']),
        ];
        let (mut runner, _) = Runner::spawn_native(&target, &[], /*target_io*/ None);
        let response = runner.request(launch_request(
            /*id*/ 33,
            stream_spec_null(),
            json!({ "mode": "denied" }),
    ...
    #[test]
    fn target_environment_preserves_caller_values_except_the_runner_namespace() {
        let fixture = cargo_bin("mcp-console-sandbox-fixture").expect("fixture binary");
        for (request_id, network) in [
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    None,
                    Some(b"/private/codex-home".to_vec()),
                    Some(b"/private/test-helper".to_vec()),
                    Some(b"1".to_vec()),
                    Some(b"1".to_vec()),
                    Some(b"1234".to_vec()),
                    Some(b"/private/managed-ca.pem".to_vec()),
                    Some(b"1".to_vec())
                ]
            );
        }
    ...
    fn observable_loader_variables_are_rejected_before_target_launch() {
        ...
        let (mut runner, _) = Runner::spawn_native(
            &target,
            &[(loader_key, "/untrusted/loader-path")],
            /*target_io*/ None,
        );
        let response = runner.request(launch_request(
            /*id*/ 43,
    ...
    fn setup_status_requires_the_policy_it_inspects() {
        ...
        assert_eq!(setup["operation"], "already_ready");
    }
    ...
    fn truncated_length_and_payload_are_rejected() {
        ...
        truncated_length.assert_control_closed();
        ...
        assert_eq!(response["id"], Value::Null);
        assert_eq!(response["error"]["code"], "malformed_frame");
        truncated_payload.assert_control_closed();
    }
    ...
    fn launch_is_invalid_without_a_native_target() {
        let mut runner = Runner::spawn(&[]);
        let response = runner.request(minimal_launch_request(/*id*/ 21));
    ...
    fn unsupported_platform_policy_is_classified_before_launch() {
        ...
        let mut launch = minimal_launch_request(/*id*/ 22);
    ...
    fn linux_rejects_graceful_termination_before_launch() {
        ...
        let target = [fixture.to_str().expect("Unicode fixture path"), "exit", "0"];
        let mut runner = Runner::spawn(&target);
        let mut launch = minimal_launch_request(/*id*/ 23);
