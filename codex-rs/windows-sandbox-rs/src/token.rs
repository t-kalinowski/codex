...
/// capability SIDs, the session-scoped logon SID, and any additional
/// non-filesystem identity SIDs.
///
/// The logon SID is required by the window-station and desktop ACLs used during
/// process initialization. Unlike the existing Codex constructors, this does
/// not add the token user or Everyone SID to `SidsToRestrict`; those identities
/// would allow a target to use host object ACLs unrelated to the normalized
/// filesystem policy.
///
/// # Safety
...
pub unsafe fn create_standalone_token_with_caps_from(
    ...
    create_token_with_caps_from_mode(
        ...
        psid_capabilities,
        additional_restricting_sids,
        BaseIdentityRestrictions::LogonOnly,
    )
}
...
unsafe fn create_token_with_caps_from_mode(
    ...
    let base_identity_count = match base_identity_restrictions {
        ...
        BaseIdentityRestrictions::LogonOnly => 1,
    };
    ...
    // the session-scoped logon SID so broad base identities cannot widen host
    // authority.
    ...
    if matches!(
        ...
        BaseIdentityRestrictions::Include | BaseIdentityRestrictions::LogonOnly
    ) {
