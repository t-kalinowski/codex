...
pub unsafe fn world_sid() -> Result<Vec<u8>> {
    unsafe { well_known_sid(WinWorldSid) }
}
...
/// Creates the standalone runner's target token from the active filesystem
/// capability SIDs, the session-scoped logon SID, the Write Restricted Code
/// SID, and any additional non-filesystem identity SIDs.
///
/// The logon SID is required by the window-station and desktop ACLs used during
/// process initialization. Write Restricted Code lets the target use Windows
/// objects whose DACL explicitly admits write-restricted code. Unlike the
/// existing Codex constructors, this does not add the token user or Everyone
/// SID to `SidsToRestrict`; those identities would allow a target to use host
/// object ACLs unrelated to the normalized filesystem policy.
///
/// # Safety
...
pub unsafe fn create_standalone_token_with_caps_from(
    ...
    create_token_with_caps_from_mode(
        ...
        psid_capabilities,
        additional_restricting_sids,
        FinalRestrictingSid::WriteRestrictedCode,
    )
}
...
unsafe fn create_token_with_caps_from_mode(
    ...
    psid_capabilities: &[*mut c_void],
    extra_restricting_sids: &[*mut c_void],
    final_restricting_sid: FinalRestrictingSid,
) -> Result<HANDLE> {
    ...
    let mut write_restricted_code = well_known_sid(WinWriteRestrictedCodeSid)?;
    ...
    // the session-scoped logon SID and Write Restricted Code so broad base
    // identities cannot widen host authority.
    ...
    entries[logon_idx + 1].Sid = match final_restricting_sid {
        FinalRestrictingSid::Everyone => psid_everyone,
        FinalRestrictingSid::WriteRestrictedCode => psid_write_restricted_code,
    };
