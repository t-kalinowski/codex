...
#[test]
fn standalone_token_includes_write_restricted_code_but_not_broad_base_identities() -> Result<()> {
    let capability_sid = LocalSid::from_string("S-1-5-21-10-20-30-40")?;
    let network_proxy_sid = LocalSid::from_string("S-1-5-21-50-60-70-80")?;
    let base_token = unsafe { get_current_token_for_restriction()? };
    ...
    let mut write_restricted_code_sid = unsafe { well_known_sid(WinWriteRestrictedCodeSid)? };
    let restricted_token = unsafe {
    ...
    let result = unsafe {
        (
            ...
            token_has_restricting_sid(
                restricted_token,
                write_restricted_code_sid.as_mut_ptr().cast(),
            )?,
