...
///
/// `CreateProcessWithLogonW` gives each suspended helper generation its own
/// logon session, and the target token retains only that session SID plus policy
/// capability SIDs. Granting and later revoking that generation-specific SID on
/// the selected Windows objects avoids adding `Everyone` or the persistent
/// sandbox account to the restricting SID set.
...
impl WindowAccessLease {
    pub(super) fn prepare(helper_process: HANDLE, desktop: StandaloneDesktop) -> Result<Self> {
        ...
        let desktop_handle = match desktop {
            StandaloneDesktop::Default => match open_default_desktop(station) {
                Ok(desktop) => desktop,
                Err(error) => {
                    if unsafe { CloseWindowStation(station) } == 0 {
                        let close_error = std::io::Error::last_os_error();
                        return Err(error).with_context(|| {
                            format!(
                                "close Winsta0 after Default desktop setup failed: {close_error}"
                            )
                        });
                    }
                    return Err(error);
    ...
    fn cleanup(&mut self) -> Result<()> {
        ...
        if self.desktop_granted {
            match revoke_access(
                ...
                Ok(()) => self.desktop_granted = false,
                Err(error) => errors.push(format!("{error:#}")),
            }
        }
        if self.station_granted {
            match revoke_access(self.station, self.logon_sid.as_mut_ptr().cast(), "Winsta0") {
                Ok(()) => self.station_granted = false,
                Err(error) => errors.push(format!("{error:#}")),
            }
        }
        self.close_revoked_handles(&mut errors);
        if errors.is_empty() {
            Ok(())
        } else {
    ...
    fn close_revoked_handles(&mut self, errors: &mut Vec<String>) {
        unsafe {
            if self.desktop != 0 && !self.desktop_granted {
                if CloseDesktop(self.desktop) == 0 {
                    errors.push(format!(
                        "CloseDesktop failed for Default desktop: {}",
                        GetLastError()
                    ));
                } else {
                    self.desktop = 0;
            ...
            if self.station != 0 && !self.station_granted {
                if CloseWindowStation(self.station) == 0 {
                    errors.push(format!(
                        "CloseWindowStation failed for Winsta0: {}",
                        GetLastError()
                    ));
                } else {
                    self.station = 0;
    ...
    fn force_close_handles(&mut self) -> Result<()> {
        let mut errors = Vec::new();
        unsafe {
            if self.desktop != 0 {
                if CloseDesktop(self.desktop) == 0 {
...
impl Drop for WindowAccessLease {
    fn drop(&mut self) {
        ...
        if let Err(error) = self.force_close_handles() {
            eprintln!("standalone Windows station handle cleanup failed: {error:#}");
        }
    }
}
...
fn open_default_desktop(station: isize) -> Result<isize> {
    ...
    if unsafe { SetProcessWindowStation(previous_station) } == 0 {
        ...
        return Err(restore_error).context("restore runner process window station");
    }
