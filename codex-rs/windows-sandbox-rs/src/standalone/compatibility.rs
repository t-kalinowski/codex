...
fn verify_helper(path: &Path, helper: WindowsSandboxStandaloneHelperKind) -> Result<()> {
    ...
    loop {
        if let Err(error) = read_available(&mut stdout, &mut stdout_bytes, &mut stdout_eof, "stdout")
            .and_then(|()| {
    ...
    let status = status.context("completed Windows helper compatibility query has no exit status")?;
