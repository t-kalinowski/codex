        ...
        loop {
            ...
            match message {
                ...
                HelperMessage::Final(mut outcome) => {
                    ...
                    outcome.cleanup_error = match process.confirm_helper_exit_after_final() {
                        Ok(helper_exit) => helper_exit.cleanup_error(),
                        Err(error) => Some(format!(
                            "standalone helper exit could not be confirmed after its final outcome: {error:#}"
                        )),
                    };
                    let mut current = observation
                        .observation
