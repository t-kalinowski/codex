...
use super::window_access::StandaloneDesktop;
use super::window_access::WindowAccessLease;
...
const HELPER_EXIT_TIMEOUT_MS: u32 = 5_000;

enum HelperExitConfirmation {
    Clean,
    ControlClosed,
    Terminated,
}

impl HelperExitConfirmation {
    fn cleanup_error(self) -> Option<String> {
        match self {
            Self::Clean => None,
            Self::ControlClosed => Some(
                "standalone helper exited only after fail-safe control-channel closure".to_string(),
            ),
            Self::Terminated => Some(
                "standalone helper did not exit within 5 seconds after its final outcome and control-channel closure; it was force-terminated"
                    .to_string(),
            ),
        }
    }
...
fn spawn_helper(
    ...
    creds: &SandboxCreds,
    desktop: StandaloneDesktop,
) -> Result<(PROCESS_INFORMATION, File, File, Vec<u8>, WindowAccessLease)> {
    ...
    let connect_result = (|| -> Result<WindowAccessLease> {
        let window_access = WindowAccessLease::prepare(process_info.hProcess, desktop)
            .context("grant standalone target access to its Windows station and desktop")?;
        process_security
        ...
        if previous_suspend_count != 1 {
            ...
            anyhow::bail!(
                "standalone helper had unexpected suspend count {previous_suspend_count}"
            );
        }
    ...
    let window_access = match connect_result {
        Ok(window_access) => window_access,
        Err(error) => {
            unsafe {
...
struct StandaloneProcessInner {
    ...
    window_access: Mutex<Option<WindowAccessLease>>,
}
...
impl StandaloneProcessInner {
    ...
    fn helper_has_exited(&self, timeout_ms: u32) -> Result<bool> {
        let helper = self.helper.as_raw_handle() as HANDLE;
        match unsafe { WaitForSingleObject(helper, timeout_ms) } {
            WAIT_OBJECT_0 => Ok(true),
            WAIT_TIMEOUT => Ok(false),
            WAIT_FAILED => Err(std::io::Error::last_os_error())
                .context("wait for standalone helper process exit"),
            result => anyhow::bail!("unexpected standalone helper wait result: {result:#x}"),
        }
    }

    fn fail_safe_retire_confirmed(&self) -> Result<HelperExitConfirmation> {
        if self.helper_has_exited(/*timeout_ms*/ 0)? {
            return Ok(HelperExitConfirmation::Clean);
        }
        self.close_control();
        if self.helper_has_exited(HELPER_EXIT_TIMEOUT_MS)? {
            return Ok(HelperExitConfirmation::ControlClosed);
        }
        let helper = self.helper.as_raw_handle() as HANDLE;
        if unsafe { TerminateProcess(helper, 1) } == 0
            && !self.helper_has_exited(/*timeout_ms*/ 0)?
        {
            return Err(std::io::Error::last_os_error())
                .context("force-terminate standalone helper process");
        }
        if !self.helper_has_exited(HELPER_EXIT_TIMEOUT_MS)? {
            anyhow::bail!("standalone helper process remained active after forced termination");
        }
        Ok(HelperExitConfirmation::Terminated)
    }

    fn confirm_helper_exit_after_final(&self) -> Result<HelperExitConfirmation> {
        // Final is the last helper message. Closing the parent-to-helper pipe is
        // therefore the causal shutdown signal for the helper's control thread,
        // not a cleanup failure. This keeps exit confirmation below the public
        // wait deadline without relying on a race between process exit and the
        // observer receiving the final frame.
        self.close_control();
        if self.helper_has_exited(HELPER_EXIT_TIMEOUT_MS)? {
            return Ok(HelperExitConfirmation::Clean);
        }
        ...
        {
            return Err(std::io::Error::last_os_error())
                .context("force-terminate standalone helper process after final outcome");
        }
        if !self.helper_has_exited(HELPER_EXIT_TIMEOUT_MS)? {
            anyhow::bail!(
                "standalone helper process remained active after final-outcome forced termination"
            );
        }
        Ok(HelperExitConfirmation::Terminated)
    }
    ...
    fn fail_safe_retire(&self) {
        if let Err(error) = self.fail_safe_retire_confirmed() {
            eprintln!("standalone helper fail-safe retirement failed: {error:#}");
        }
        if let Err(error) = self.release_window_access() {
            eprintln!("standalone Windows station cleanup failed: {error:#}");
        }
    }

    fn release_window_access(&self) -> Result<()> {
        let window_access = self
            .window_access
            .lock()
            .map_err(|_| anyhow::anyhow!("standalone window-access lock poisoned"))?
            .take();
        match window_access {
            Some(window_access) => window_access.release(),
            None => Ok(()),
        }
    }
}
...
fn publish_control_failure(
    ...
    process: &StandaloneProcessInner,
    error: anyhow::Error,
) {
    let helper_exit = match process.fail_safe_retire_confirmed() {
        Ok(helper_exit) => helper_exit,
        Err(helper_error) => {
            eprintln!(
                "standalone helper outcome and exit could not be confirmed: {error:#}; {helper_error:#}"
            );
            return;
        }
    };
    ...
    let window_cleanup = process
        .release_window_access()
        .err()
        .map(|error| format!("Windows station cleanup failed: {error:#}"));
    current.final_outcome = Some(WindowsSandboxStandaloneOutcome {
        ...
        cleanup_error: combine_cleanup_errors(helper_exit.cleanup_error(), window_cleanup),
    });
...
fn observe_helper(
    ...
    let result = (|| -> Result<()> {
        loop {
            ...
            match message {
                ...
                HelperMessage::Final(mut outcome) => {
                    {
                    ...
                    let helper_exit = match process.confirm_helper_exit_after_final() {
                        Ok(helper_exit) => helper_exit,
                        Err(error) => {
                            eprintln!(
                                "standalone helper final outcome cannot be published because process exit was not confirmed: {error:#}"
                            );
                            return Ok(());
                        }
                    };
                    ...
                    outcome.cleanup_error =
                        combine_cleanup_errors(helper_exit.cleanup_error(), window_cleanup);
                    let mut current = observation
                        .observation
                        .lock()
                        .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    if let Err(error) = result {
        publish_control_failure(&observation, &process, error);
    }
}
...
pub fn spawn_windows_sandbox_standalone(
    ...
    let desktop = if request.use_private_desktop {
        StandaloneDesktop::Private
    } else {
        StandaloneDesktop::Default
    };
    let (process_info, control, events, runner_sid, window_access) =
        spawn_helper(&request.setup, &creds, desktop)?;
    ...
    let inner = Arc::new(StandaloneProcessInner {
        ...
        window_access: Mutex::new(Some(window_access)),
    });
