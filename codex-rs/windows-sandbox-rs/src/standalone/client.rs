    ...
    fn cleanup_error(self) -> Option<String> {
        match self {
            ...
            Self::ControlClosed => Some(
                "standalone helper did not exit after its final outcome within 5 seconds; its control channel was closed"
                    .to_string(),
            ),
            Self::Terminated => Some(
                "standalone helper did not exit after its final outcome and control-channel closure; it was force-terminated"
                    .to_string(),
            ),
        }
    }
...
impl StandaloneProcessInner {
    ...
    fn confirm_helper_exit_after_final(&self) -> Result<HelperExitConfirmation> {
        if self.helper_has_exited(HELPER_EXIT_TIMEOUT_MS)? {
            Ok(HelperExitConfirmation::Clean)
        } else {
            self.fail_safe_retire_confirmed()
        }
    }
