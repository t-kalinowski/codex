...
use crate::identity::sandbox_creds_are_current;
...
pub fn windows_sandbox_standalone_setup_status(
    ...
    match load_prepared_sandbox_creds(
        ...
        Ok(Some(credentials))
            if credentials.username
                ...
                && sandbox_creds_are_current(&credentials) => {}
        Ok(_) => {
            return WindowsSandboxStandaloneSetupState::AdministrativeActionRequired {
                reason: "sandbox credentials are missing, stale, or incompatible".to_string(),
            };
        }
        Err(error) => {
            return WindowsSandboxStandaloneSetupState::Unavailable {
    ...
    let payload = match setup_payload(request, /*refresh_only*/ true) {
        Ok(payload) => payload,
        Err(error) => {
            return WindowsSandboxStandaloneSetupState::Unavailable {
                reason: format!("failed to resolve requested filesystem setup policy: {error}"),
            };
        }
    };
    if !marker.standalone_filesystem_policy_matches(&payload) {
        return WindowsSandboxStandaloneSetupState::RefreshRequired {
            reason: "filesystem ACL policy differs from the last successful setup generation"
                .to_string(),
        };
    }
...
pub fn windows_sandbox_standalone_verified_setup_status(
    ...
    match local {
        WindowsSandboxStandaloneSetupState::Ready
        | WindowsSandboxStandaloneSetupState::RefreshRequired { .. } => {}
        WindowsSandboxStandaloneSetupState::AdministrativeActionRequired { .. }
        | WindowsSandboxStandaloneSetupState::Unavailable { .. } => return local,
    }
    ...
    match verify_windows_sandbox_standalone_network_with_policy_lease(request, &policy_lease) {
        ...
        Err(error) => WindowsSandboxStandaloneSetupState::AdministrativeActionRequired {
            reason: format!(
                "installed Windows sandbox firewall policy requires refresh: {error:#}"
            ),
        },
    }
}
    ...
    match windows_sandbox_standalone_setup_status(request) {
        ...
        WindowsSandboxStandaloneSetupState::RefreshRequired { .. } => {
                ...
                .is_ok()
            {
                ...
                match windows_sandbox_standalone_setup_status(request) {
                    WindowsSandboxStandaloneSetupState::Ready => {
                        return Ok(WindowsSandboxStandaloneSetupOperation::Refreshed);
                    }
                    state => anyhow::bail!(
                        "standalone Windows filesystem setup remained stale after refresh: {state:?}"
                    ),
                }
            }
        }
...
pub fn refresh_windows_sandbox_standalone(
    ...
    match windows_sandbox_standalone_setup_status(request) {
        WindowsSandboxStandaloneSetupState::Ready
        | WindowsSandboxStandaloneSetupState::RefreshRequired { .. } => {}
        state => anyhow::bail!("standalone Windows setup changed before refresh: {state:?}"),
    }
    verify_windows_sandbox_standalone_network_with_policy_lease(request, &policy_lease)
    ...
    match windows_sandbox_standalone_setup_status(request) {
        WindowsSandboxStandaloneSetupState::Ready => {}
        state => anyhow::bail!("standalone Windows setup remained stale after refresh: {state:?}"),
    }
