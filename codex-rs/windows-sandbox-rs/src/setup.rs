...
impl SetupMarker {
    ...
    pub(crate) fn standalone_filesystem_policy_matches(&self, payload: &ElevationPayload) -> bool {
        self.command_cwd.as_deref().is_some_and(|command_cwd| {
            canonical_path_key(command_cwd) == canonical_path_key(&payload.command_cwd)
        }) && path_sets_match(&self.read_roots, &payload.read_roots)
            && path_sets_match(&self.write_roots, &payload.write_roots)
            && path_sets_match(&self.deny_read_paths, &payload.deny_read_paths)
            && path_sets_match(&self.deny_write_paths, &payload.deny_write_paths)
    }
...
fn path_sets_match(left: &[PathBuf], right: &[PathBuf]) -> bool {
    left.iter()
