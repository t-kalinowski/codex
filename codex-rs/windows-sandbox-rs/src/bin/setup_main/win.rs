...
fn real_main() -> Result<()> {
    ...
    let (payload_b64, verify_network) = match args.as_slice() {
        ...
        [_, switch, payload_b64]
            if switch == WINDOWS_SANDBOX_STANDALONE_VERIFY_NETWORK_SWITCH =>
        {
            (payload_b64, true)
        }
        _ => {
...
fn run_provision_only(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    let sandbox_group_sid = resolve_sandbox_users_group_sid(payload.policy_namespace).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            SetupErrorCode::HelperSidResolveFailed,
