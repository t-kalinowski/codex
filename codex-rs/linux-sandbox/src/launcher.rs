...
pub(crate) fn exec_bwrap(
    ...
    bwrap_info_fd: Option<std::os::fd::RawFd>,
) -> ! {
    ...
    apply_process_options(&mut argv, monitor_standard_streams, bwrap_info_fd);
...
fn apply_process_options(
    ...
    let mut options = vec![OsString::from("--as-pid-1")];
    if let Some(info_fd) = bwrap_info_fd {
        options.extend([
            OsString::from("--info-fd"),
            OsString::from(info_fd.to_string()),
        ]);
    }
    if matches!(monitor_standard_streams, MonitorStandardStreams::Release) {
        options.push(OsString::from(CLOSE_MONITOR_STANDARD_STREAMS_ARG));
    }
    argv.splice(1..1, options);
}
...
mod tests {
    ...
    fn default_process_options_preserve_monitor_standard_streams() {
        ...
        apply_process_options(&mut argv, MonitorStandardStreams::Preserve, None);
    ...
    fn embedding_process_options_release_monitor_standard_streams() {
        ...
        apply_process_options(&mut argv, MonitorStandardStreams::Release, None);
    ...
    #[test]
    fn embedding_process_options_request_host_process_information() {
        let mut argv = vec![OsString::from("bwrap"), OsString::from("--")];

        apply_process_options(&mut argv, MonitorStandardStreams::Release, Some(197));
