...
pub fn verify_packaged_bwrap_runtime(
    ...
    let embedding = prepare_packaged_bwrap(&runner_executable, application_state_dir)?;
    let runner_read_path = AbsolutePathBuf::from_absolute_path(&runner_executable)
        .map_err(|err| format!("embedding executable must be absolute: {err}"))?;
    ...
    let mut child = command
        .spawn()
        .map_err(|err| format!("failed to start packaged Linux namespace preflight: {err}"))?;
        ...
        match runtime_preflight_has_exited(&child) {
            Ok(true) => {
                ...
                let status = child.wait().map_err(|err| {
                    format!("failed to reap packaged Linux namespace preflight: {err}")
                })?;
                if status.success() {
                    return Ok(());
                }
            ...
            Ok(false) => {
                ...
                return Err("packaged Linux namespace preflight timed out".to_string());
            }
            Err(err) => {
                terminate_runtime_preflight(&mut child);
                return Err(format!(
